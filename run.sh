...
maindir=""
container_name=scalable-mpc-container
network_name=scalable-mpc-network
...
while test "$#" -ne 0; do
    if test "$1" = "-C" -o "$1" = "--clean"; then
        ...
        if test "$arch" = "arm64" -o "$arch" = "aarch64"; then
            ...
            echo "\`run.sh --arm\` only works on ARM64 hosts" 1>&2
            exit 1
        fi
        ...
        echo "Usage: run.sh [-C|--clean]$armtext [-V|--verbose]" 1>&2
        exit 1
    fi
done
...
if test -z "$tag" -a "$platform" = linux/arm64; then
    tag=scalable-mpc:arm64
elif test -z "$tag"; then
    tag=scalable-mpc:latest
fi
...
create_network() {
    echo "scalable-mpc network not found, creating one..."
    docker network create -d bridge scalable-mpc-network
}

remove_containers() {
    echo "Removing all existing scalable-mpc containers..."
    docker ps -a -f name=scalable-mpc --format "{{.ID}}" | while read line ; do docker rm --force $line ; done
}
...
while test "$dir" != / -a "$dir" != ""; do
    thisfileid=`stat $statformatarg %d:%i "$dir"/run.sh 2>/dev/null`
...
if test -n "$maindir"; then
    ...
    if test -n "$existing_image"; then
        ...
        echo "- To start a new container, exit then \`run.sh -f\`" 1>&2
